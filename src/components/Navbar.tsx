...
import { Menu, X, ArrowRight, Sparkles, PhoneCall, Layers, ShieldCheck, ChevronRight } from "lucide-react";
            ...
            <nav className="hidden lg:flex items-center gap-1 bg-[#071B4D]/40 border border-white/[0.08] px-4 py-1.5 rounded-full backdrop-blur-md">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="px-3.5 py-1.5 text-xs font-semibold text-slate-300 hover:text-white rounded-full hover:bg-white/[0.06] transition-all duration-200 flex items-center gap-1.5"
                >
                  <span>{link.name}</span>
                  {link.badge && (
                    <span className="px-1.5 py-0.5 text-[9px] font-bold bg-blue-500/20 text-blue-400 border border-blue-400/30 rounded-full uppercase tracking-wider">
                      {link.badge}
                    </span>
                  )}
                </Link>
            ...
            <div className="space-y-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="flex items-center justify-between px-4 py-3 text-sm font-semibold text-slate-200 hover:text-white hover:bg-white/[0.05] rounded-xl transition-all"
                >
